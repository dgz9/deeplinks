...
// Icon paths for QR code logo generation
...
1. Your YouTube handle (e.g., '@channelname')
2. Your channel path (e.g., 'channel/UCxxxxxxx')`,
    icon: SiYoutube,
    color: '#FF0000'
  },
  snapchat: {
    inputLabel: 'Username',
    placeholder: 'username without @',
    instructions: 'Enter your Snapchat username without the @ symbol',
    icon: SiSnapchat,
    color: '#FFFC00'
  },
  facebook: {
...
4. Scroll down to "Page ID"`,
    icon: SiFacebook,
    color: '#1877F2'
  },
  linkedin: {
    inputLabel: 'Profile URL or ID',
    placeholder: 'company/name or in/username',
    instructions: `Enter your LinkedIn URL segment:
1. For personal profile: 'in/username' (from linkedin.com/in/username)
2. For company page: 'company/name' (from linkedin.com/company/name)`,
    icon: SiLinkedin,
    color: '#0A66C2'
  },
  tiktok: {
...
function App() {
  ...
  const generateLinks = (e) => {
    ...
    const linkData = {
      ...
      linkedin: {
        deepLink: null, // LinkedIn deep linking is unreliable, so we'll disable it
        webLink: /^\d+$/.test(username)
      ...
      youtube: {
        deepLink: username.startsWith('@')
          ? `vnd.youtube://www.youtube.com/${username}`
          : `vnd.youtube://www.youtube.com/${username}`,
        webLink: `https://www.youtube.com/${username}`
      },
      snapchat: {
        deepLink: `snapchat://add/${username}`,
        webLink: `https://snapchat.com/add/${username}`
      }
    }
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        qrImage.onload = async () => {
          ...
          <path fill="${platformInfo[platform].color}" d="${getIconPath(platform)}"/>
        </svg>`;
      ...
      <div className="max-w-md mx-auto bg-white rounded-2xl shadow-xl p-8">
        ...
        {links && (
          <div className="mt-8 space-y-6">
            <Card>
              <CardBody>
                <h2 className="text-sm font-medium text-gray-500 mb-2">Deep Link {platform !== 'linkedin' && `(${isAndroid ? 'Android' : 'iOS'})`}</h2>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className={`space-y-3 ${!links.deepLink ? "col-span-2" : ""}`}>
